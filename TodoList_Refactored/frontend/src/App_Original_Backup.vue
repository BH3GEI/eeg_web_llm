    ...
    <main class="main">
      ...
      <div v-if="currentMode === 'chat'" class="chat-container">
        ...
        <div class="parent-selection">
          <label>选择你的赛博家长：</label>
          <div class="parent-buttons">
            <button @click="selectedParent = 'dad'" :class="{ active: selectedParent === 'dad' }" class="parent-btn">
               唠叨老爸
            </button>
            <button @click="selectedParent = 'mom'" :class="{ active: selectedParent === 'mom' }" class="parent-btn">
               关怀老妈
            </button>
          </div>
        </div>
          ...
          <div class="data-quality-indicator">
            ...
            <span class="quality-text">
              {{ focusMode.dataQuality === 'good' ? '真实EEG数据' : 
                 focusMode.dataQuality === 'simulated' ? '' : '回退数据' }}
            </span>
          </div>
...
export default {
  ...
  methods: {
    ...
    async sendChatMessage() {
      ...
      try {
        const response = await axios.post('/api/chat', {
          ...
          message: userMessage,
          parent_type: this.selectedParent  // 传递父母角色
        })
    ...
    startNewConversation() {
      ...
      // 生成新的会话ID - 完全新的session
      this.sessionId = 'session_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9)
      localStorage.setItem('smart_todo_session_id', this.sessionId)
      
      // 显示确认消息
      this.showNotification('已开始全新对话')
    },
...
<style scoped>
/* CYBERPARENTS 真正的赛博朋克配色方案 */
:root {
  /* 主色调 - 真正的黑色背景 */
  --cyber-black: #000000;
  --cyber-dark-gray: #1C1C1C;
  --cyber-medium-gray: #2A2A2A;
  --cyber-light-gray: #3A3A3A;
  
  /* 霓虹强调色 */
  --cyber-neon-cyan: #00FFFF;
  --cyber-neon-blue: #0099CC;
  --cyber-neon-purple: #9B59B6;
  --cyber-neon-pink: #FF69B4;
  --cyber-neon-green: #39FF14;
  
  /* 文字色调 - 高对比度 */
  --cyber-text-primary: #FFFFFF;
  --cyber-text-secondary: #E0E0E0;
  --cyber-text-muted: #A0A0A0;
  --cyber-text-dark: #000000;
  
  /* 背景渐变 */
  --cyber-bg-primary: linear-gradient(135deg, #000000 0%, #1C1C1C 50%, #2A2A2A 100%);
  --cyber-bg-secondary: linear-gradient(135deg, #1C1C1C 0%, #2A2A2A 100%);
  
  /* 玻璃态效果 - 更高对比度 */
  --cyber-glass: rgba(0, 255, 255, 0.15);
  --cyber-glass-dark: rgba(28, 28, 28, 0.9);
  --cyber-glass-border: rgba(0, 255, 255, 0.4);
  
  /* 阴影效果 */
  --cyber-shadow-neon: 0 0 20px rgba(0, 255, 255, 0.6);
  --cyber-shadow-glow: 0 0 40px rgba(155, 89, 182, 0.4);
}
...
@keyframes neon-text-glow {
  0%, 100% { 
    text-shadow: 0 0 10px var(--cyber-neon-cyan), 0 0 20px var(--cyber-neon-cyan), 0 0 30px var(--cyber-neon-cyan);
  }
  50% { 
    text-shadow: 0 0 20px var(--cyber-neon-purple), 0 0 30px var(--cyber-neon-purple), 0 0 40px var(--cyber-neon-purple);
  }
}
...
.header h1 {
  ...
  text-shadow: 0 0 20px var(--cyber-neon-cyan), 0 0 40px var(--cyber-neon-cyan);
  letter-spacing: 4px;
...
.header h1::before {
  ...
  text-shadow: 0 0 20px var(--cyber-neon-purple);
  opacity: 0.3;
  animation: neon-text-glow 3s ease-in-out infinite reverse;
}
...
.mode-btn.active {
  ...
  box-shadow: 0 0 30px var(--cyber-neon-blue), 0 0 60px var(--cyber-neon-purple);
